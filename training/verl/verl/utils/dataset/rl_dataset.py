...
class RLHFDataset(Dataset):
    ...
    def _build_messages(self, example: dict):
        ...
        # Runtime skill-bank injection (evolving-skilled variant). When extra_info
        # carries a "skill_bank_path", read it fresh on every __getitem__ and
        # append the level's skills block to the system message before the chat
        # template is applied. No-op for variants that do not set this field.
        extra_info = example.get("extra_info") or {}
        skill_bank_path = extra_info.get("skill_bank_path")
        if skill_bank_path:
            level_name = extra_info.get("level_name")
            try:
                from verl_tool.utils.skill_injection import (
                    format_skills_block,
                    load_skill_bank,
                )

                bank = load_skill_bank(skill_bank_path)
                block = format_skills_block(level_name, bank) if level_name else ""
                if block:
                    for msg in messages:
                ...
                # Bank file may briefly not exist if the evolution hook is mid-rewrite.
                # Atomic os.rename in the hook should prevent this in practice; tolerate it anyway.
                logger.warning("skill_bank_path not found: %s — falling back to base prompt", skill_bank_path)
            except Exception as e:
                logger.warning("skill injection failed (%s: %s) — falling back to base prompt", type(e).__name__, e)
