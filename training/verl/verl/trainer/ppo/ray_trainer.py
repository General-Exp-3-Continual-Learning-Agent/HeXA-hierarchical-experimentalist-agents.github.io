    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # Step-end skill-evolution hook (continuous-process evolving variant).
                # Fires synchronously after the policy update + checkpoint save have
                # completed. Driver process is idle here so a blocking HTTP call to
                # the teacher vLLM is safe. No-op for all other variants.
                from omegaconf import OmegaConf
                _skill_evolve_freq = int(OmegaConf.select(self.config, "trainer.skill_evolve_freq", default=0) or 0)
                if _skill_evolve_freq > 0 and self.global_steps % _skill_evolve_freq == 0:
                    with marked_timer("skill_evolution", timing_raw, color="cyan"):
                        try:
                            from verl_tool.hooks.skill_evolution import run_skill_evolution
                            run_skill_evolution(
                                skill_bank_path=OmegaConf.select(self.config, "trainer.skill_bank_path"),
                                level_name=OmegaConf.select(self.config, "trainer.skill_evolve_level"),
                                teacher_endpoint=OmegaConf.select(self.config, "trainer.skill_evolve_teacher_endpoint"),
                                work_dir=OmegaConf.select(self.config, "trainer.skill_evolve_work_dir", default=None),
                                max_skills=int(OmegaConf.select(self.config, "trainer.skill_evolve_max_skills", default=10)),
                                max_mistakes=int(OmegaConf.select(self.config, "trainer.skill_evolve_max_mistakes", default=5)),
                                global_step=self.global_steps,
                            )
                        except Exception as e:
                            print(f"[skill_evolution] hook raised {type(e).__name__}: {e} — continuing training")
